...
public class StateFileDAO implements StateDAO {
	...
	public void init() {
		...
		try (
			...
			while ((line = buf.readLine()) != null) {
				...
				String dessert = tokens[7];
				states.add(new State(id, abbrev, name, capital, latitude, longitude, population, dessert));
			}
		} catch (Exception e) {
	...
	public State getStateById(int id) {
		State s = null;
		System.out.println("get state by id " + id);
		for (State state : states) {
			if (state.getId() == id) {
				s = state;
				System.out.println("assigned state = " + s.getName());
				break;
			}
		}
		return s;
